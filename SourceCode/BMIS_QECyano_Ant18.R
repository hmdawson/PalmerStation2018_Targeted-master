...
samp.key.file <- "Metadata/SampleList_HannahAnt18_RP.csv"
is.names.file <- "Metadata/InternalStandardNames_Ant18_total.csv"
xcms.dat.file <- "Intermediates/PostQCforBMIS_RP_Ant18.csv"
cut.off <- 0.2
cut.off2 <- 0.1
...
IS_names <- read_csv(is.names.file) %>%
  filter(Internal_Standards != "Thiamine Hydrochloride, 13C3") %>%
  filter(Internal_Standards != "Tryptamine Hydrochloride, D4")%>%
  filter(Internal_Standards != "Riboflavin-dioxopyrimidine, 13C4, 15N2")
...
xcms.dat <- xcms.dat %>%
  filter(!str_detect(`Precursor.Ion.Name`, "Thiamine Hydrochloride, 13C3")) %>%
  filter(!str_detect(`Precursor.Ion.Name`, "Tryptamine Hydrochloride, D4"))%>%
  filter(!str_detect(`Precursor.Ion.Name`, "Riboflavin-dioxopyrimidine, 13C4, 15N2"))
...
IS.dat <- IS.dat %>% mutate(Replicate.Name = `Replicate.Name` %>%
                              str_replace("-","."))  %>%
  select(Filled_area, Replicate.Name, MassFeature)
xcms.long <- xcms.dat %>%
  rename(Replicate.Name = `Replicate.Name`,
         MassFeature = `Precursor.Ion.Name`) %>%
  select(Replicate.Name, MassFeature, Filled_area)
xcms.long <- xcms.long %>%
