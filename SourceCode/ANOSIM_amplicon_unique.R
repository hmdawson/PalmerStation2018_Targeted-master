...
abu.file <- "Intermediates/18S_unique_raw_cleaned.csv"

#Load up meta.dat
meta.dat <- read_csv(meta.dat.file) %>%
  filter(!str_detect(SampID, "Slush|EvXSW_A|B1_E|15|Dock|B1_A"))
...
speabu.wide <- speabu.wide %>%
  filter(!str_detect(SampleID, "Dockslush_A|Dockslush_B|Dockslush_C|EvXSW_A|Ev51Slush_A|Ev15SW_A|B1_A" ))
...
# 
# y.anosim<-anosim(speabu.d,wide.meta$Sample_type, permutations = 19999)
# 
# summary(y.anosim)
# 
...
speabu.dd<-vegdist(newdata[,1:1222], "bray") 
yy.anosim<-anosim(speabu.dd,newdata$Site, permutations = 19999)
summary(yy.anosim)
...
#Name inputs-----
meta.dat.file <- "Metadata/Ant18_metadata_plots.csv"
abu.file <- "Intermediates/16S_unique_raw_cleaned.csv"
...
newdata$Sample_type<-factor(newdata$Sample_type)
speabu.dd<-vegdist(newdata[,1:1350], "bray")
yy.anosim<-anosim(speabu.dd,newdata$Sample_type, permutations = 19999)
summary(yy.anosim)
plot(yy.anosim)
