...
#Get the wide data
dat.filename <- "RawOutput/AllData_Final_QC_data.csv"
...
long.dat <- dat %>%
  ...
  select(MassFeature, Carbon.Norm.Area, SampID, Sample_type, Exp_group)
...
for(i in seq_along(Exp.List)){
  ...
  anovas <- Exp.List[[i]] %>%
    split(.$MassFeature) %>%
    map(~aov(Carbon.Norm.Area ~ Sample_type, data = .)) %>% 
    map(summary) 
...
write(List.of.MFs, file = paste(file_prefix, Sys.Date(), "anovas_MassFeatures_untarg_fdr.txt", sep = "_"),
      ncolumns = if(is.character(List.of.MFs)) 1 else 5,
      append = FALSE, sep = " ")

write.csv(list.both, file = paste(file_prefix, Sys.Date(), "anovas_MassFeatures_untarg_fdr.csv", sep = "_"))
