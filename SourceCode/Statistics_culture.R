...
wdat <- dat %>%
  ...
  dplyr::select(Identification, nmolperumolCinEnviroave, SampID) %>%
...
long.dat <- dat %>%
  ...
  dplyr::select(Identification, nmolperumolCinEnviroave, SampID, Sample_type, Exp_group)
...
for(i in seq_along(Exp.List)){
  ...
  anovas <- Exp.List[[i]] %>%
    ...
    map(~aov(nmolperumolCinEnviroave ~ Sample_type, data = .)) %>% 
    map(summary) 
...
write.csv(list.both, file = paste(file_prefix, Sys.Date(), "anovas_MassFeatures_fdr.csv", sep = "_"))
