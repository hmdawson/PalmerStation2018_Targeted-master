...
#Changes HMD (1/22/2021),
##Commented out replicate min
##updated file names for R project structure
##updated parameters
##issue with EV60SW, not integrated in positive, filtered out in negative for now as well line 65
##fixed IS issue with succinic acid with new skyline IS output (replaced IS from main file with new from Laura)

##For this sample set skip replicate min ssince some samples like sea ice don't have replicates
...
Datfile1 <- "RawOutput/HILIC_QE_NEGATIVE_Ant18.csv"
ISdatfile <- "RawOutput/HILIC_QE_NEGATIVE_Ant18_UpdatedInternalStandards.csv"
...
#What's your output file name? (specify whether you want comments)
fileout <- "Intermediates/QCd_HILICNeg_Ant18.csv"
fileout.comment <- "Intermediates/QCd_HILICNeg_Ant18_comment.csv"
...
#Combine all the Datfiles (specify old or new runs) and add on IS data from separate skyline file
Datfile_IS <- read.csv(ISdatfile)%>%
  filter(!str_detect(Replicate.Name,"Ev60SW"))

Datfile_Comb_pre <- read.csv(Datfile1) %>%
  filter(!str_detect(Replicate.Name,"Ev60SW"))%>%
  filter(!str_detect(Protein,"Internal-Standards_neg"))
  
Datfile_Comb <- rbind(Datfile_Comb_pre,Datfile_IS)%>%
  unique()
