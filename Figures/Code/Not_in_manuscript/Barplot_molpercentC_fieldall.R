...
dat <- read_csv(dat.filename) %>%
  filter(str_detect(`SampID`, "StaB|Hero|Ev"))
...
dat.prep <- dat %>%
  ...
  dplyr::select(CultureID, Identification, molFractionC) %>%
...
b.all <- ggplot()+
  geom_bar(stat = "identity", position = "fill", data = dat.mean.combo, 
           aes(x = CultureID_short, y = molFractionC, fill = Identification), color = "black", size = 0.2)+
...
save_plot("Figures/Preliminary/barplot_molFractionC_fieldall.pdf", b.all, base_height = 10, base_width = 15)
