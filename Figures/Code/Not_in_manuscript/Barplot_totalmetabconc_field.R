...
dat <- read_csv(dat.filename) %>%
  filter(!str_detect(`SampID`, "ppt|Ev"))
...
Total_plot <-ggplot(Total, aes(x=Sample_type, y=ave)) +
  ...
  ggtitle("Field total metabolite pools") +
  labs(x="Station",y= bquote('nmol C metabolite / µmol C'))

Total_plot  
save_plot("Figures/Preliminary/barplot_nMolCperumolC_total_field.pdf", Total_plot, base_height = 5, base_width = 7)
