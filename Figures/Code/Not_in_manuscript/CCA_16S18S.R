...
#file names
speabu16S.file <- "Rawoutput/16S/16S_phyla.csv"
speabu18S.file <- "Rawoutput/18S/18S_phyla.csv"
...
#axis length for DCA1 is 0.8 stdevs - suggests linear response model appropriate - RDA
decorana(speabu.16.dat, ira=0)
...
spe.rda<-rda(speabu.16.dat~., speabu.18.dat)
summary(spe.rda)
...
spe.rda0<-rda(speabu.16.dat~1,speabu.18.dat) 
...
text(spe.rda, choices=c(1,2), labels=row.names(speabu.18.dat))
...
pdf('RDA16S18S.pdf', width=10, height=10)
plot(spe.rda, choices=c(1,2), display=c('wa', 'sp', 'bp'), scaling=2)
