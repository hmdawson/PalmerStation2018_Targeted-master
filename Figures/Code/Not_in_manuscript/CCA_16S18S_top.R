...
#file names
speabu16S.file <- "Rawoutput/16S/16S_taxa_top20.csv"
speabu18S.file <- "Rawoutput/18S/18S_taxa_top20.csv"

#import data sets (had to delete first row of sheet manually)
speabu.16.dat <- read.csv(speabu16S.file, header=TRUE, row.names=1)


speabu.18.dat <- read.csv(speabu18S.file, header=TRUE, row.names=1)
speabu.18.dat <-  speabu.18.dat %>% mutate(SampleID = rownames(speabu.18.dat))%>%
...
#axis length for DCA1 is 2.2 stdevs - suggests unimodal response model appropriate - CCA
decorana(speabu.18.dat, ira=0)
...
spe.cca<-cca(speabu.18.dat~., speabu.16.dat)
summary(spe.cca)
...
spe.cca0<-cca(speabu.16.dat~1,speabu.18.dat) 
ordiR2step(spe.cca0,spe.cca)

#ordination scores
#visualize ordination by plotting just the site (wa) scores
plot(spe.cca, choices=c(1,2), type='points', display='wa', scaling=2)

#add site labels
plot(spe.cca, choices=c(1,2), type='n', display='wa', scaling=2)
text(spe.cca, choices=c(1,2), labels=row.names(speabu.18.dat))
...
pdf('cca16S18S_top.pdf', width=15, height=10)
plot(spe.cca, choices=c(1,2), display=c('wa', 'sp', 'bp'), scaling=2)
...
plot(spe.cca,choices=c(1,2),type='none',scaling=2) 
points(spe.cca,choices=c(1,2),display='wa',pch=19,cex=1.5,scaling=2) 
text(spe.cca,choices=c(1,2),display='sp',col='red',cex=.75,scaling=2) 
text(spe.cca,choices=c(1,2),display='bp',col='blue')
