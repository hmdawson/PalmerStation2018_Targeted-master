...
dat.tran2 <- data.stand(wide.matrix.2, method ='total', margin='column', plot=F)
# 
# #NMDS
...
nmds.raw<-metaMDS(t(dat.tran2), distance='bray', k=2, autotransform=FALSE, wascores = FALSE, noshare = FALSE, trymax=999)
monte.pvalue.raw <-nmds.monte(t(dat.tran2), distance='bray', k=2, autotransform=FALSE, trymax=20)
monte.pvalue.result.raw <- monte.pvalue.raw[[2]]
print(paste(monte.pvalue.result.raw, "= pvalue of nmds"))
pointlocation.nmds.raw <- spe.nmds[['points']] %>% as.data.frame() %>%
  mutate(SampID = rownames(spe.nmds[['points']])) %>%
  left_join(meta.dat, by = "SampID") 

# plot(nmds.raw, type='n', main= "NMDS plot")
# text(nmds.raw, labels=row.names(t(wide.matrix.2)))
...
save_plot("Figures/Preliminary/NMDS_18S_all.pdf", d.raw, base_height = 5, base_width = 7)
