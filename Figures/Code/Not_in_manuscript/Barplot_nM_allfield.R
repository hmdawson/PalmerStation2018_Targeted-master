...
dat <- read_csv(dat.filename) %>%
  filter(str_detect(`SampID`, "Sta|Ev|Hero"))
...
b.all.2
save_plot("Figures/Preliminary/barplot_nM_field.pdf", b.all.2, base_height = 10, base_width = 10)
...
b.all <- ggplot()+
  geom_bar(stat = "identity", position = "fill", data = dat.mean.combo, 
           aes(x = CultureID_short, y = nmolinEnviroave, fill = Identification), color = "black", size = 0.2)+
