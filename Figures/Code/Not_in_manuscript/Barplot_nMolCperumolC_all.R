...
#Load up files----
#remove sea ice samples since they can't be normalized to carbon
...
order.of.compounds <- dat.mean %>% ungroup %>% 
  arrange(FigureID, desc(nmolCave)) %>%
  group_by(FigureID) %>%
...
dat.mean.others <-  dat.mean %>%
  filter(!Identification %in% order.of.compounds.2$Identification) %>%
  group_by(FigureID, Sample_type, total_mmol) %>%
...
b.all.2 <- ggplot()+
  geom_bar(stat = "identity", data = dat.mean.combo, 
           aes(x = FigureID, y = nmolCave, fill = Identification), color = "black", size = 0.2)+
  scale_y_continuous(expand = c(0, 0), limits = c(0,45))+
  scale_fill_manual(values = pal)+
  labs(y = bquote('Mean nmol C umol'*~POC^-1*''))+
...
save_plot("Figures/Preliminary/Draft_MS/Figure_6_stackedbar.pdf", b.all.2, base_height = 10, base_width = 14)


#save plot using cairo_pdf to get unicode symbols for N and S
library(ggplot2)
cairo_pdf("Figures/Preliminary/Draft_MS/Figure_6_stackedbar.pdf", height = 10, width = 14)
ggplot()+
  geom_bar(stat = "identity", data = dat.mean.combo, 
           aes(x = FigureID, y = nmolCave, fill = Identification), color = "black", size = 0.2)+
  scale_y_continuous(expand = c(0, 0), limits = c(0,45))+
...
# b.all
# 
# save_plot("Figures/Preliminary/barplot_nMolCperumolC_field_proportional.pdf", b.all, base_height = 10, base_width = 10)
# 
