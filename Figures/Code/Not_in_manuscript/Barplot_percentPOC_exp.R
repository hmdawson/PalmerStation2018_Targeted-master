...
dat.prep <- dat %>%
  ...
  dplyr::select(CultureID, Identification, molFractionC_pertotalC) %>%
...
b.all.2 <- ggplot()+
  geom_bar(stat = "identity", data = dat.mean.combo, 
           aes(x = Org_Name, y = molFractionC_pertotalC, fill = Identification), color = "black", size = 0.2)+
...
b.all.2
save_plot("Figures/Preliminary/barplot_percentPOC_exp.pdf", b.all.2, base_height = 10, base_width = 10)
