...
dat.prep <- dat %>%
  ...
  dplyr::select(CultureID, Identification, molFractionN) %>%
...
dat.mean <- dat.prep %>% 
  ...
  mutate(molFractionN = ifelse(is.na(molFractionN), 0 , molFractionN)) %>%
  summarise(molFractionN = mean(molFractionN)) %>%
  ungroup() %>%
  group_by(CultureID_short) %>%
  mutate(total_mmol = sum(molFractionN))
...
order.of.compounds <- dat.mean %>% ungroup %>% 
  arrange(CultureID_short, desc(molFractionN)) %>%
  group_by(CultureID_short) %>%
  mutate(ID_rank = rank(desc(molFractionN))) %>%
  mutate(top_ten = ifelse(ID_rank < 16, ID_rank, NA))
...
dat.mean.others <-  dat.mean %>%
  ...
  summarise(molFractionN = sum(molFractionN)) %>%
...
#plot
pal <- c(colorRampPalette(brewer.pal(8,"Dark2"))(19)[1:19], rep("grey", 1))

b.all <- ggplot()+
  geom_bar(stat = "identity", position = "fill", data = dat.mean.combo, 
           aes(x = Org_name_plots, y = molFractionN, fill = Identification), color = "black", size = 0.2)+
  scale_y_continuous(expand = c(0, 0))+
  scale_fill_manual(values = pal)+
  labs(y = bquote('mole fraction N') )+
...
save_plot("Figures/Preliminary/barplot_molfractionN_field.pdf", b.all, base_height = 10, base_width = 10)
