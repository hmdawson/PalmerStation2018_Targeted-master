...
dat.mean <- dat.prep %>% 
  group_by(SampID, Identification, CultureID_short, Sample_type) %>%
...
write.csv(dat.mean, "Tables/nMCtotal_all.csv")
...
b.all.2
save_plot("Figures/Preliminary/barplot_nmolC_all.pdf", b.all.2, base_height = 10, base_width = 14)
...
b.all <- ggplot()+
  ...
  scale_fill_manual(values = pal)+
  labs(y = bquote('nMole C '*~L^-1*' (proportional)') )+
  theme(legend.title = element_blank(),
        legend.text = element_text(size = 10),
        legend.position="bottom",
        legend.justification = "center",
...
save_plot("Figures/Preliminary/barplot_nmolC_field_proportional.pdf", b.all, base_height = 10, base_width = 10)
