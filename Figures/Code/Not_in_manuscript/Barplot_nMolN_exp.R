...
dat.prep <- dat %>%
  ...
  dplyr::select(CultureID, Identification, nmolNave) %>%
...
order.of.compounds <- dat.mean %>% ungroup %>% 
  arrange(CultureID_short, desc(nmolNave)) %>%
  group_by(CultureID_short) %>%
  mutate(ID_rank = rank(desc(nmolNave))) %>%
...
dat.mean.others <-  dat.mean %>%
  ...
  summarise(nmolNave = sum(nmolNave)) %>%
...
b.all.2 <- ggplot()+
  geom_bar(stat = "identity", data = dat.mean.combo, 
           aes(x = Org_Name, y = nmolNave, fill = Identification), color = "black", size = 0.2)+
  scale_y_continuous(expand = c(0, 0), limits = c(0,1200))+
  scale_fill_manual(values = pal)+
  labs(y = bquote('nMole N '*~L^-1*''))+
...
b.all.2
save_plot("Figures/Preliminary/barplot_nmolN_exp.pdf", b.all.2, base_height = 10, base_width = 10)
...
b.all <- ggplot()+
  geom_bar(stat = "identity", position = "fill", data = dat.mean.combo, 
           aes(x = Org_name_plots, y = nmolNave, fill = Identification), color = "black", size = 0.2)+
  scale_y_continuous(expand = c(0, 0))+
  scale_fill_manual(values = pal)+
  labs(y = bquote('nMole N '*~L^-1*' (proportional)') )+
  theme(legend.title = element_blank(),
        legend.text = element_text(size = 10),
        legend.position="bottom",
        legend.justification = "center",
...
save_plot("Figures/Preliminary/barplot_nmolN_exp_proportional.pdf", b.all, base_height = 10, base_width = 10)
