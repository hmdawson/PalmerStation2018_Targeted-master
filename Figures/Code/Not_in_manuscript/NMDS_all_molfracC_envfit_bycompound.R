...
nmds.raw<-metaMDS(wide.matrix.2.raw, distance='euclidean', k=2, autotransform=FALSE, wascores = FALSE, noshare = FALSE, trymax=999)
...
d.raw <- ggplot(data = pointlocation.nmds.raw, aes(x =MDS1, y =  MDS2, group = SampID,
                                                   colour = SampID,
                                                   fill = SampID,
                                                   label = SampID))+
  geom_point(size = 3)+
  geom_text(size=2, colour="black")+
  geom_polygon(aes(fill = SampID), alpha = 0.2) +
...
save_plot("Figures/Preliminary/NMDS_bycompound_molfracC_field.pdf", d.raw, base_height = 5, base_width = 25)
