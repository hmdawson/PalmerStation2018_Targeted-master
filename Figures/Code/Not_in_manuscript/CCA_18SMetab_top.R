...
speabu.dat <- read.csv(speabu18S.file, header=TRUE, row.names=1)
#speabu.dat <- speabu.dat[-(1:9),] 
metab.dat <- read.csv(metabtop20.file, header=TRUE, row.names=1)
#metab.dat <- metab.dat[-(1:9),] 
...
spe.rda<-rda(metab.dat~., speabu.dat)
summary(spe.rda)
...
spe.rda0<-rda(metab.dat~1,speabu.dat) 
...
pdf('RDAMetab18S_top.pdf', width=10, height=10)
plot(spe.rda, choices=c(1,2), display=c('wa', 'sp', 'bp'), scaling=2)
...
points(spe.rda,choices=c(1,2),display='wa',pch=19,cex=1.5,scaling=1) 
text(spe.rda,choices=c(1,2),display='sp',col='red',cex=.75,scaling=1) 
