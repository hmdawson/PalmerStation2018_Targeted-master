...
dat <- read_csv(dat.filename) %>%
  filter(str_detect(`SampID`, "Sta|Hero"))
...
dat.prep <- dat %>%
  ...
  dplyr::select(CultureID, Identification, nmolperumolCinEnviroave) %>%
...
dat.mean <- dat.prep %>% 
  ...
  mutate(nmolperumolCinEnviroave = ifelse(is.na(nmolperumolCinEnviroave), 0 , nmolperumolCinEnviroave)) %>%
  summarise(nmolperumolCinEnviroave = mean(nmolperumolCinEnviroave)) %>%
  ungroup() %>%
  group_by(CultureID_short) %>%
  mutate(total_mmol = sum(nmolperumolCinEnviroave))
...
order.of.compounds <- dat.mean %>% ungroup %>% 
  arrange(CultureID_short, desc(nmolperumolCinEnviroave)) %>%
  group_by(CultureID_short) %>%
  mutate(ID_rank = rank(desc(nmolperumolCinEnviroave))) %>%
...
dat.mean.others <-  dat.mean %>%
  ...
  summarise(nmolperumolCinEnviroave = sum(nmolperumolCinEnviroave)) %>%
...
#non-proportional
pal <- c(colorRampPalette(brewer.pal(8,"Dark2"))(17)[1:17], rep("grey", 1))

b.all.2 <- ggplot()+
  geom_bar(stat = "identity", data = dat.mean.combo, 
           aes(x = Org_Name, y = nmolperumolCinEnviroave, fill = Identification), color = "black", size = 0.2)+
  scale_y_continuous(expand = c(0, 0), limits = c(0,8.5))+
  scale_fill_manual(values = pal)+
  labs(y = bquote('nMole '*~umolPOC^-1*''))+
...
b.all.2
save_plot("Figures/Preliminary/barplot_nMolperumolC_field_all.pdf", b.all.2, base_height = 10, base_width = 10)

#Proportional
pal <- c(colorRampPalette(brewer.pal(8,"Dark2"))(14)[1:14], rep("grey", 1))

b.all <- ggplot()+
  geom_bar(stat = "identity", position = "fill", data = dat.mean.combo, 
           aes(x = Org_name_plots, y = nmolperumolCinEnviroave, fill = Identification), color = "black", size = 0.2)+
  scale_y_continuous(expand = c(0, 0))+
  scale_fill_manual(values = pal)+
  labs(y = bquote('nMole '*~umolPOC^-1*' (proportional)') )+
  theme(legend.title = element_blank(),
        legend.text = element_text(size = 10),
        legend.position="bottom",
        legend.justification = "center",
...
save_plot("Figures/Preliminary/barplot_nMolperumolC_field_proportional.pdf", b.all, base_height = 10, base_width = 10)
