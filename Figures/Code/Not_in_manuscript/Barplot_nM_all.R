...
dat <- read_csv(dat.filename) %>%
  filter(!str_detect(`SampID`, "EvXSW_A|Ev51Slush_A|Ev15SW_A|EvXCore_A"))
...
dat.prep <- dat %>%
  ...
  dplyr::select(SampID, Identification, nmolinEnviroave) %>%
...
order.of.compounds <- dat.mean %>% ungroup %>% 
  ...
  mutate(top_ten = ifelse(ID_rank < 10, ID_rank, NA))
...
#non-proportional
pal <- c(colorRampPalette(brewer.pal(8,"Dark2"))(18)[1:18], rep("grey", 1))

b.all.2 <- ggplot()+
  geom_bar(stat = "identity", data = dat.mean.combo, 
           aes(x = Sample_type, y = nmolinEnviroave, fill = Identification), color = "black", size = 0.2)+
  scale_y_continuous(expand = c(0, 0), limits = c(0,1100))+
  scale_fill_manual(values = pal)+
  labs(y = bquote('nMole '*~L^-1*''))+
...
b.all.2
save_plot("Figures/Preliminary/barplot_nM_all.pdf", b.all.2, base_height = 10, base_width = 10)
...
b.all <- ggplot()+
  geom_bar(stat = "identity", position = "fill", data = dat.mean.combo, 
           aes(x = Sample_type_plots, y = nmolinEnviroave, fill = Identification), color = "black", size = 0.2)+
  scale_y_continuous(expand = c(0, 0))+
  scale_fill_manual(values = pal)+
  labs(y = bquote('nMole '*~L^-1*' (proportional)') )+
  theme(legend.title = element_blank(),
        legend.text = element_text(size = 10),
        legend.position="bottom",
        legend.justification = "center",
...
save_plot("Figures/Preliminary/barplot_nM_all_proportional.pdf", b.all, base_height = 10, base_width = 10)
