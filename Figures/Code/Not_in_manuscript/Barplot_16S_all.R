...
order.of.taxa <- dat.prep%>% ungroup %>% 
  ...
  mutate(top_ten = ifelse(ID_rank < 43, ID_rank, NA))
...
pal <- c(colorRampPalette(brewer.pal(8,"Dark2"))(7)[1:7], rep("grey", 1))
...
stacked.relative <- ggplot()+
  ...
  labs(x = "", y = "Relative Abundance (%)", title= "16S relative abudance", fill = "Class")

stacked.relative

save_plot("Figures/Preliminary/barplot_16S_class_all_facet.png", stacked.relative, base_height = 10, base_width = 15)
...
save_plot("Figures/Preliminary/barplot_16S_class_all.png", stacked.relative, base_height = 10, base_width = 15)
