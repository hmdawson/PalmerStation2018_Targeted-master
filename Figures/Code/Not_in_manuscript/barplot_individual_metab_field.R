...
long.dat <- read_csv(quan.file)%>%
  filter(!str_detect(SampID, c("EvXSW_A|Ev60SW_A|Ev51Slush_A|Ev15SW_A|StaB1_D|StaB1_E")))%>%
  left_join(meta.dat, by = "SampID")

#Pick compound
DOC <- long.dat%>%
  filter(Identification == "Aspartic acid")
...
save_plot("Figures/Preliminary/barplot_expfield_Glutamine.pdf", TankGR_plot, base_height = 7, base_width = 9)
