...
speabu.dat <- read.csv(speabu16S.file, header=TRUE, row.names=1)
speabu.dat <-  speabu.dat %>% mutate(SampleID = rownames(speabu.dat))%>%
...
metab.dat <-  metab.dat %>% mutate(SampleID = rownames(metab.dat))%>%
  filter(!str_detect(SampleID, "StaB1_D|StaB1_E" ))%>%
  dplyr::select(-SampleID)
...
pdf('RDAMetab16S.pdf', width=10, height=10)
plot(spe.rda, choices=c(1,2), display=c('wa', 'sp', 'bp'), scaling=2)
