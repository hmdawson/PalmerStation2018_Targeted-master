...
wide.matrix.2.raw <- data.stand((wide.matrix.2), method='total', margin='row', plot=F)%>% mutate(names = row.names(wide.matrix.2))
...
metab_tile_field <- ggplot(stat = "identity", data = datwidestd, aes(x = SampID, y = Identification , fill = std_conc, color="")) +
  geom_tile(fill = NA) +
  geom_tile(colour = NA) +
  theme_minimal()+
...
save_plot("Figures/Preliminary/Tile_field_total.pdf", metab_tile_field, base_height = 10, base_width = 10)
