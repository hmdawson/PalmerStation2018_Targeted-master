...
unique.select <- unique.select[!grepl('core_seawater', rownames(unique.select)),]
...
bp(s.melt, "16S Relative Abundance")
...
#just incubation
 r.inc <- r[grep('Carboy', rownames(r)),]
...
#just field
  r.field <- r[grep('B|core|Hero', rownames(r)),]
...
data <- metadata[grep('B|Carboy|core|Hero', rownames(metadata)),]
data[,1:3] <- NULL
data <- data[-1,] 
data <- data[-30,] 
