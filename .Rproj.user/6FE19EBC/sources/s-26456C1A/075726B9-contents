...
xcms.dat.pos.file <- "Intermediates/QCd_HILICPos_Ant18.csv"
xcms.dat.neg.file <- "Intermediates/QCd_HILICNeg_Ant18.csv" 
cut.off <- 0.2
cut.off2 <- 0.1
...
xcms.dat <- rbind(xcms.dat_pos, xcms.dat_neg) %>%
    ...
    mutate(QC_area = as.numeric(QC_area))
xcms.dat <- xcms.dat %>%
...
SampKey <- SampKey_all %>%
  ...
  select(`Replicate.Name`, QC_area, MassFeature)

IS.dat <- rbind(IS.dat, SampKey) %>% mutate(Column = "HILICPos") %>%
  mutate(QC_area = ifelse(is.na(QC_area), 0, QC_area))


#Look at extraction replication of the Internal Standards----
IS_inspectPlot <- ggplot(IS.dat, aes(x=`Replicate.Name`, y=QC_area)) + 
...
IS.dat <- IS.dat %>% mutate(Replicate.Name = `Replicate.Name` %>%
                              str_replace("-","."))  %>%
  select(QC_area, Replicate.Name, MassFeature, Column)
xcms.long <- xcms.dat %>%
  rename(Replicate.Name = `Replicate.Name`,
         MassFeature = `Precursor.Ion.Name`) %>%
  select(Replicate.Name, MassFeature,  Column, QC_area)
xcms.long <- xcms.long %>%
...
IS.means <- IS.dat %>% filter(!grepl("_Blk_", Replicate.Name)) %>%
  ...
  summarise(ave = mean(as.numeric(QC_area))) %>%
...
for (i in 1:length(unique(IS.dat$MassFeature))){
  Split_Dat[[i]] <- binded %>% mutate(MIS = unique(IS.dat$MassFeature)[i]) %>%
    left_join(IS.dat %>% 
                rename(MIS = MassFeature, IS_Area = QC_area) %>% 
